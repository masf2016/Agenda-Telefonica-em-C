...
| Data:      19/05/2013                             |
| Descricao: Exemplificacao de lista encadeada em C |
+==================================================*/
...
void inserefim(void) {
    ...
    //O último valor, será Nulo, e depois apontando para o Final
    aux->proximo=NULL;
    aux->proximo=final;
}
...
void ultelemento(void){
    Dados *aux=principal;
    //Enquanto o próximo elemento não for NULL Avance uma posição
    while(aux->proximo!=NULL){
        aux=aux->proximo;
    }
    fprintf(stdout, "------------------------\n");  
